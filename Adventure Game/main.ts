...
let enemies: Array<string> = [ "Skeleton", "Zombie", "Warrior", "Assassin" ];
...
while (running) {
    console.log("----------------------------------------");
    let enemyHealth:number = Math.floor(Math.random() * (maxEnemyHealth + 1)) + 0;
    let enemy:string = enemies[Math.floor(Math.random() * (enemies.length + 1)) + 0]
